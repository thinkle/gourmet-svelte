    ...
    <div slot="right">
      <NavActions>
        {#if $shoppingList.length > 0}
          <Button
            toggle="true"
            toggled={showSubItems}
            on:click={() => (showSubItems = !showSubItems)}>Show details</Button
          >
        {/if}
        <IconButton
          toggle="true"
        ...
        {:else}
          {#await saving}
            <Button width="5em" busy={true}>Saving...</Button>
          {:then}
            <IconButton icon="close" on:click={() => (saving = undefined)}>
              All set!
              {(function () {
                saving = false;
              })()}
          ...
          {/await}
        {/if}
        <RecipePickerLauncher onSelected={shoppingList.addRecipe}>
  ...
  <FullHeight scrolls={true}>
    {#if $recipesOnList && $recipesOnList.length > 0}
      <h3 on:click={() => (showRecipes = !showRecipes)}>
        For {$recipesOnList.length}
        {#if $recipesOnList.length > 1}Recipes{:else}Recipe{/if}
      </h3>
      {#if showRecipes}
    ...
    {/if}
    <div>
      <ShoppingListItems {showSubItems} items={$shoppingList} />
    </div>
    {#if DEV}
