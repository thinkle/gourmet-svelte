...
export function markupTextForIngredient(text, ing) {
  ...
  for (let i = 0; i <= 3; i++) {
    // up to three highlights per ingredient only
    let highlighter = highlighters[i];
    if (highlighter) {
      text = text.replace(highlighter.matcher, function (v) {
...
export function getItemMatchers(text) {
  ...
  for (let si = 0; si < words.length; si++) {
    // start index
    for (let ei = si + 1; ei < words.length + 1; ei++) {
      // end index...
      let matcher = new RegExp(
        "\\b" + words.slice(si, ei).join("[\\s,.-]+") + "\\b",
        "gi"
      );
      if (text.match(matcher)) {
        matchers.push({ matcher, length: ei - si });
      }
    }
  }
...
export function extractItems(text) {
  ...
  words = stopword.removeStopwords(words, [/chop(s|ped)?/i, /mince[sd]?/i]);
  return words;
}

export function getNutritionQuery(text, aggressive = true) {
  let words = text.split(/[^\w\u00C0-\u017F]+/);
  ...
  if (aggressive) {
    const removeWords = ["babyfood", "snack", "prepared", "mix"];
