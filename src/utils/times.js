...
const timeWords = [
  { matcher: /\b(sec(ond)?s?)\b/i, name: "second", value: 1 },
  { matcher: /\b(min(ute)?s?)\b/i, name: "minute", value: M },
  { matcher: /\b(h(ou)?rs?)\b/i, name: "hour", value: H },
  { matcher: /\b(da?ys?)\b/i, name: "day", value: D },
  { matcher: /\b(we+ks?)\b/i, name: "week", value: W },
  { matcher: /\b(mo?n?ths)\b/i, name: "month", value: MNTH },
  { matcher: /\b(y(ea)?rs?)\b/i, name: "year", value: YR },
];
...
function buildAtLeastOneTimeMatcher() {
  ...
  for (let requiredUnit of timeWords) {
    ...
    for (let i = timeWords.length - 1; i >= stopAt; i--) {
      ...
      let unitMatcher =
        "\\d+\\s*" + reToString(unit.matcher).replace(/\\b/g, "");
...
const timeMatcher = /((\band|\W+\b)?\s*\d+\s[acdehikmnorstuw]+\s*)+/gi; // Much simplified -- less precise, but maybe won't leaad to catastrophic failure?
const fancyTimeMatcher = new RegExp(
  reToString(timeMatcher).replace("\\d+", numMatchString),
  "ig"
);
...
export default {
  ...
  getScale(s) {
    if (s <= 60) {
      ...
      // 5 year timer
      return { m: H * 24 * 365 * 5, u: (H * 24 * 365) / 4 }; // quarter year units
    }
  },

  getHMS(seconds) {
    if (isNaN(seconds)) {
      new Error(`getHMS requires a number but got ${seconds}`);
    }
  ...
  getDescription(s) {
    ...
    if (!hms.hours) {
      ...
      if ([0, 15, 30, 45].indexOf(hms.seconds) > -1) {
        ...
        return `${floatToFrac(hms.minutes)} minutes, ${floatToFrac(
          hms.seconds,
          { fallbackDigits: 0 }
        )} seconds`;
      }
    ...
    if (hms.hours < 24) {
      if ([0, 15, 30, 45].indexOf(hms.minutes) > -1) {
        ...
        return `${floatToFrac(hms.hours)} hour ${floatToFrac(
          hms.minutes
        )} minutes`;
      }
