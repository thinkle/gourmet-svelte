...
import { shoppingList, storedShopRec } from "./shoppingStores";
...
describe("Starting empty", () => {
  ...
  xit("Create empty shopping list", async () => {
    shoppingList.get();
  ...
  it("Add custom item to shopping list", async () => {
    await shoppingList.get();
    let id = await shoppingList.addItem({
      amount: { amount: 1, unit: "can" },
      text: "tomatoes",
    });
    expect(id).toBeDefined();
    let id2 = await shoppingList.addItem({
      amount: { amount: 1, unit: "can" },
      text: "beans",
    });
...
describe("Starting full", () => {
  ...
  it("Create new shopping list with some recipes in it", async () => {
    await shoppingList.get();
  ...
  it("Delete item from shopping list", async () => {
    await shoppingList.get();
    await shoppingList.clear();
    let recipeIds = await getSomeIDs();
    await shoppingList.addRecipe(recipeIds[3]);
