<script>
  ...
  import { NumberUnitDisplay, IconButton } from "../../widgets/";
  ...
  $: highlightersActive =
    ...
    $highlightedIngredient.active &&
    $highlightedIngredient.active.indexOf(ing.text) > -1;

  function toggleHighlight(ingredient) {
    if (!highlightersActive) {
      return;
    }
...
{#if ing}
  <tr
    class="ing"
    class:ingref={ing.reference}
    class:highlighted={$highlightedIngredient &&
      ing.text == $highlightedIngredient.highlighted}
    class:hover={$highlightedIngredient &&
      ing.text == $highlightedIngredient.hover}
  ...
  >
    {#if edit}
      <IngredientEditor
        ...
        bind:ing
      />
    {:else if ing.reference}
      <!-- Recipes as ingredients (embedded...) -->
      <NumberUnitDisplay mode="table" value={ing.amount} />
      <td class="link">
        <a
          target="_blank"
          href={`${baseUrl}${ing.reference}`}
          on:click={handleReferenceClick}>{ing.text}</a
        >
      </td>
    {:else}
      <!-- Standard Ingredient -->
