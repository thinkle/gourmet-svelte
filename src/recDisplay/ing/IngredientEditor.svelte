...
{#if ing.reference}
  <NumberUnitInput bind:value={ing.amount} {onChange} />
  <td>
    ...
    <Underline
      ...
      /></Underline
    >
    <RecipePickerLauncher onSelected={addRecipeReference}>
      (Change Recipe)
...
{:else}
  <!-- Editor -->
  <td colspan="3">
    <IngredientInput {onChange} {onDelete} {onEnter} {shouldFocus} {ing} />
  </td>
{/if}
<td style="border:none">
  <Menu bind:this={menu} icon="more_vert" anchorRight={true}>
    {#if position > 0}
      <MenuItem icon="arrow_upward" on:click={() => menu.hideMenu(onMove(-1))}>
        Move Up
      </MenuItem>
    {/if}
    {#if position < parent?.length - 1}
      <MenuItem icon="arrow_downward" on:click={() => menu.hideMenu(onMove(1))}>
        Move Down
      </MenuItem>
    {/if}
    {#if topLevel && (!ing.amount || (!ing.amount.amount && !ing.amount.unit))}
      <MenuItem
        icon="collections"
        on:click={() => {
          menu.hideMenu(onChange({ ...ing, ingredients: [] }));
        }}
      >
        Make into group
      </MenuItem>
    {/if}
    <MenuItem icon="delete" on:click={onDelete}>Delete</MenuItem>
    <MenuItem>
      <RecipePickerLauncher
        onSelected={(id) => menu.hideMenu(addRecipeReference(id))}
        onClose={menu.hideMenu}>Add Recipe as Ingredient</RecipePickerLauncher
      >
    </MenuItem>
    {#if !topLevel}
      <MenuItem
