...
import { titleCase, cleanupWhitespace } from "../utils/textUtils";
import {
  handleIngredientAmount,
  handleIngredientUnit,
  handleIngredient,
  handleIngredientText,
  handleIngredientGroup,
  handleIngredients,
} from "./ingredientImporter.js";
import { handleLDJson } from "./jsonImporter";
import { handleTime } from "./timeImporter";
...
export function preprocessChunks(parsedChunks, context) {
  ...
  parsedChunks = parsedChunks.map((o) => ({ ...o }));
  let uniqueOnly = [];
  for (let chunk of parsedChunks) {
    if (uniqueOnly.length == 0) {
      ...
      console.log(
        ...
        chunk,
        "duplicates",
        uniqueOnly[uniqueOnly.length - 1]
      );
    }
  }
  ...
  context.chunkMap = {};
  parsedChunks.forEach((chunk) => (context.chunkMap[chunk.id] = chunk));
  return parsedChunks;
}
  ...
  for (let childIndex = 1; childIndex < parsedChunks.length; childIndex++) {
    for (let parentIndex = childIndex - 1; parentIndex > -1; parentIndex--) {
      ...
      if (
        childAddress &&
        parentAddress &&
        childAddress.indexOf(parentAddress) == 0
      ) {
        // i.e. 001-002-003 matches 001-002
  ...
  for (let { name, array, match } of RecDef.recProps) {
    if (other[name]) {
      if (!orig[name]) {
        ...
        orig[name] = mergeArrayVals(orig[name], other[name], match);
      }
    }
  }
  orig.ingredients = mergeIngredients(orig.ingredients, other.ingredients);
  return orig;
}
...
export function ignoreMatchingDescendants(
  chunk,
  context,
  { extraTagsToIgnore, ignoreAllTags } = {}
) {
  if (!chunk) {
    return;
  }
    ...
    chunk.children.forEach((child) => {
      if (context.chunkMap[child]) {
        ...
        if (
          ignoreAllTags ||
          ch.tag == chunk.tag ||
          (extraTagsToIgnore && extraTagsToIgnore.includes(ch.tag))
        ) {
          ch.handled = true;
          ignoreMatchingDescendants(ch, context, {
            extraTagsToIgnore,
            ignoreAllTags,
          }); // recursive!
        }
...
function handleCategory(chunk, context, recipe) {
  ...
  recipe.categories.push({ name: cleanupWhitespace(chunk.text) });
}
...
function handleImage(chunk, context, recipe) {
  ...
  if (img) {
    recipe.images.push({ url: img.src, alt: img.alt });
  }
}
...
function getUrl(
  chunk,
  { selector = "a[href]", attr = "href", baseUrl = "" } = {}
) {
...
function addSourceIfMissing(recipe, context) {
  if (context.url) {
    ...
    if (!alreadyHaveUrl) {
      recipe.sources.push({ name: context.host, url: context.url });
    }
  }
}

function extractUrlFromText(text) {
  let liberalUrlMatcher =
