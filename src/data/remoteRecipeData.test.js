...
import { RecipeData } from "./remoteRecipeData";
import { testRecs } from "../common/mocks/recipes";
import user from "../common/mocks/user";
import { mockLambdaFunction } from "./functions/mockFunction";
import { setupEmptyDB, setupDBwithRecs } from "./functions/setupMockDB";
import deepcopy from "deepcopy";
import { jsonConcisify } from "../utils/textUtils";
let recipeData = RecipeData(user);
...
it("connect to db", async () => {
  await recipeData.connect();
});

it("Get recs", async () => {
  mockLambdaFunction(user);
  let response = await recipeData.getRecipes({ limit: 2 });
});

it("Add rec, get _id, retrieve by _id", async () => {
  let recipe = await recipeData.addRecipe(deepcopy(testRecs.standard));
  ...
  expect(recipe.ingredients.length).toEqual(
  ...
  // }
  let fetchedRec = await recipeData.getRecipe(recipe._id);
...
it("Store and retrieve a recipe", async () => {
  const recipe = await recipeData.addRecipe({ ...testRecs.standard });
  expect(recipe._id).toBeDefined();
  //expect(recipe.id).toEqual(testRecs.standard.id);
  expect(recipe.owner.email).toEqual(user.email);
  const readRec = await recipeData.getRecipe(recipe._id);
...
it("Store multiple recs", async () => {
  ...
  await recipeData.addRecipe({ ...testRecs.standard });
  await recipeData.addRecipe({ ...testRecs.standard });
  await recipeData.addRecipe({ ...testRecs.standard });
  let resp = await recipeData.getRecipes(undefined);
...
it("Update recipe", async () => {
  ...
  let rec = await recipeData.addRecipe(testRecs.standard);
  r.title = "Updated " + r.title;
  r.id = rec.id;
  r._id = rec._id;
  let r2 = await recipeData.updateRecipe(r);
  ...
  let rfresh = await recipeData.getRecipe(r._id);
