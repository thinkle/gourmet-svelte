<script>
  ...
  import { user, redirectURL } from "../stores/userStore";
  ...
  let functions = [
    {
      ...
      action: () => {
        promise = queryNutrientRequest.makeRequest({
          params: { query: "baked potato" },
        });
      },
    },
    {
      ...
      action: () => {
        promise = getNutrientInfoRequest.makeRequest({
          params: { id: "1450081" },
        });
      },
    },
    ...
    },
    {
      ...
      action: () => {
        promise = remoteRecipeData.getRecipe(theOne._id);
      },
    },
    {
      name: "Update the one",
      action: () => {
        ...
        promise = remoteRecipeData.updateRecipe(theOne);
      },
    },
    {
      name: "Fetch 10",
...
<div>
  ...
  {#if promise}
    ...
    {:then data}
      <h3>Got data :)</h3>
      <button on:click={() => (savedResults = [data, ...savedResults])}
        >save</button
      >
      <div>{showData(data)}</div>
      <p on:click={() => (theOne = data)}>Make "the one"</p>
      {#if data.result}
        {#if Array.isArray(data.result)}
          ...
          {/each}
        {:else}
          <div on:click={() => (theOne = data.result)}>
            .result: {showData(data.result)}
          </div>
        {/if}
      {/if}
    {:catch err}
