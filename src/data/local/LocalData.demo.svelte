<script>
  import localApi from "./dexieApi";
  import { JsonDebug, Button } from "../../widgets";
  async function addRec() {
    results = await localApi.addRecipe({
      title: "Hello world",
      savedRemote: 0,
    });
  }
...
<br /><Button on:click={async () => (results = await localApi.getRecipe(1))}
  >Get Rec #1</Button
>
...
<br /><Button
  on:click={async () =>
    (results = await localApi.getRecipes({
      limit: 50,
      query: { deleted: 0 },
      sort: sortProp,
    }))}>Get sorted recs</Button
> <input bind:value={sortProp} />
<br /><Button
  on:click={async () => {
    debugger;
    results = await localApi.getRecipes({
