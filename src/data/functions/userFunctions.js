...
removeLinkedAccountRequest.setRequestHandler(
...
acceptLinkedAccountRequest.setRequestHandler(
  cache(async function (user, params) {
    ...
    if (
      otherAccount &&
      otherAccount.linkedAccounts &&
      otherAccount.linkedAccounts.includes(user.email)
    ) {
  ...
  cache(async function (user, params) {
    ...
    if (dbUser.linkedAccounts) {
      if (!Array.isArray(dbUser.linkedAccounts)) {
        console.log(
          "bad linkedAccounts stored: not an array",
          dbUser.linkedAccounts
        );
        dbUser.linkedAccounts = [];
      }
      dbUser.linkedAccounts = [
        ...new Set([...dbUser.linkedAccounts, ...linkedAccounts]),
      ];
    } else {
