<script>
 ...
 $: if (!newName) {
     newName = $user.remoteUser ? $user.remoteUser.name : $user.name
 }
...
<div class="block">
    ...
    {#if $user && $user.remoteUser && $user.remoteUser.dbUser && $user.remoteUser.dbUser.invites}
        ...
        {/if}
        {#if $user.remoteUser.dbUser.invites.length > 1}
        ...
        {/if}
        {#each $user.remoteUser.dbUser.invites as invite}
    ...
    {#if $user &&$user.remoteUser && $user.remoteUser.dbUser && $user.remoteUser.dbUser.linkedAccounts}
